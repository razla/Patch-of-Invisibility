...
Gparser.add_argument('--seed', default='7986',type=int, help='choose seed')
...
Gparser.add_argument('--method', default='nes_est', type=str, help='options: raw, random_raw, random_gan, bbgan, wbgan, nes, nes_est, square')
...
# [0, 9, 47, 1526, 1581, 2115, 3004, 4490, 5964]  # YOLOv5 best seeds
# [452, 4702]  # YOLOv4 best seeds
seeds = [2115, 3004, 4490, 5964]
...
for epoch in range(n_epochs):
    ...
    for i_batch, (img_batch, lab_batch) in tqdm(enumerate(train_loader), desc=f'2 Running epoch {epoch}',total=epoch_length_second):
        ...
        best_seed, best_loss = seeds[0], 1.0
        for s in seeds:
            apply_seed(s)
            latent_vector = torch.normal(0.0, torch.ones(len_latent)).to(device).requires_grad_(True)
            ...
            with torch.autograd.set_detect_anomaly(True):
                ...
                if best_loss > loss_det:
                    best_loss = loss_det
                    best_seed = s
                    print(f"!!!!!*********** best_seed {s}, best_loss {best_loss} ***********")
                print(f"*********** seed {s}, loss {loss_det} ***********")
        exit(0)
