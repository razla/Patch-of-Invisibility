...
source_folder = "/cs_storage/public_datasets/inria/inria/Test/pos/"  # ../dataset/inria/Train/pos/   ,   ../dataset/inria/Test/pos/
# video or folder
...
if yolo_tiny == True and model_name != "yolov2":
    ...
    sss = model_name
# label_labelRescale_folder = ("./dataset/inria/Test/pos/yolo-labels-rescale_" + sss).replace("yolov5", "yolov4")
label_labelRescale_folder = os.path.join(source_folder, f"yolo-labels-rescale_{sss}").replace("yolov5", "yolov4")
enable_show_map_process = False

# st()
# output path
output_video_name = "video_output"
output_folder = "/home/eylonmiz/NatadvPatch/eval_output/" + sss + "/"
output_video_foler = output_folder + "video/"
output_imgs_and_labels_foler = output_folder + "output_imgs_and_labels/"
output_labels_foler = os.path.join(output_imgs_and_labels_foler, "yolo-labels/")
outout_labelRescale_folder = os.path.join(output_imgs_and_labels_foler, "yolo-labels-rescale/")
...
tiny_str = "tiny" if yolo_tiny else ""
if (yolo_tiny):
...
if (model_name == "yolov2"):
    ...
    output_labels_foler = output_labels_foler[:-1] + "_yolov5" + tiny_str + output_labels_foler[-1]
    outout_labelRescale_folder = outout_labelRescale_folder[:-1] + "_yolov5" + tiny_str + output_labels_foler[-1]
...
if (enable_output_data):
    ...
    os.makedirs(output_imgs_and_labels_foler, exist_ok=True)
...
### -----------------------------------------------------------    Detector    ---------------------------------------------------------------------- ###
# select detector
detector = load_detector(model_name, yolo_tiny, 1, True)
# if (model_name == "yolov2"):
#     detectorYolov2 = DetectorYolov2(show_detail=False, tiny=yolo_tiny, epoch_save=1, multi_score=True)
#     detector = detectorYolov2
# if (model_name == "yolov3"):
#     detectorYolov3 = DetectorYolov3(show_detail=False, tiny=yolo_tiny, epoch_save=1, multi_score=True)
#     detector = detectorYolov3
# if (model_name == "yolov4"):
#     detectorYolov4 = DetectorYolov4(show_detail=False, tiny=yolo_tiny, epoch_save=1, multi_score=True)
...
for i, imm in tqdm(enumerate(source_data), desc=f'Output video ', total=nframes):
    ...
    max_prob_obj_cls, overlap_score, bboxes = None, None, None
    if model_name == "yolov3" or model_name == "yolov2" or model_name == "yolov4":
        max_prob_obj_cls, output_class_target, bboxes = detector.detect(input_imgs=imm_tensor,
                                                                cls_id_attacked=cls_id_attacked, epoch=1, with_bbox=enable_with_bbox)
    elif model_name == "yolov5":
        max_prob_obj_cls, bboxes = detector.detect(imm_tensor, cls_id_attacked=cls_id_attacked, is_train=False)
        # max_prob_obj_cls = torch.tensor([p if p.numel() > 0 else 0.0 for p in max_prob_obj_cls], device=max_prob_obj_cls[0].device)
    ...
    if (model_name == "yolov3" or model_name == "yolov4"):
        ...
        for b in bbox:
            if (int(b[-1]) == int(cls_id_attacked)):
                b = b.detach().cpu().numpy()
                # cls_id, xc, yc, w, h, obj_conf
                label = np.array([b[-1], ((b[0] + b[2]) / 2.0) / img_side, ((b[1] + b[3]) / 2.0) / img_side, (b[2] - b[0]) / img_side, (b[3] - b[1]) / img_side, b[4]],
                                 dtype=np.float32)
                labels.append(label)
                # cls_id, obj_conf*cls_conf, x, y, x, y 
                label_rescale = np.array([b[-1], b[-2], b[0], b[1], b[2], b[3]], dtype=np.float32)
...
############################################
    ...
    if (len(labels) > 0):
        ...
        loss_det, p_img_batch, bboxes = eval_rowPtach(input_imgs=imm_tensor, label=labels_tensor,
    ...
    if (enable_output_data):
        # save clear imgs
        output_path = str(output_imgs_and_labels_foler) + '%s.png' % (iname)
...
if (enable_count_map):
    if not (enable_show_map_process):
        output_imgs_and_labels_foler = None
    # st()
    output_map = eval_map.count(path_ground_truth=label_labelRescale_folder,
                                path_detection_results=outout_labelRescale_folder,
                                path_images_optional=output_imgs_and_labels_foler)
    # save
    with open(os.path.join(output_folder, "map.txt"), "w") as text_file:
