...
def interpolate_shift(G, z, latent_vector, device='cpu'):
    if not(z.size() == latent_vector.size()):
        latent_vector_masked = torch.zeros_like(z).to(device)
        latent_vector = latent_vector_masked
    latent_vector = latent_vector.cuda()
    shifted_image = G.gen_shifted(z, latent_vector).cpu()[0]
    return shifted_image

def evo_interpolate_shift(G, z, latent_vector):
    latent_vector = latent_vector.cuda()
    shifted_image = G.gen_shifted(z, latent_vector).cpu()
    return shifted_image
