...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Compute conf
        obj_conf = deepcopy(x[:, 5]).unsqueeze(-1)  # objectness
        ...
        # Detections matrix nx6 (xyxy, obj, obj*conf, cls)
        if multi_label:
            i, j = (x[:, 5:mi] > conf_thres).nonzero(as_tuple=False).T
            x = torch.cat((box[i], obj_conf, x[i, 5 + j, None], j[:, None].float(), mask[i]), 1)
        else:  # best class only
            conf, j = x[:, 5:mi].max(1, keepdim=True)
            x = torch.cat((box, obj_conf, conf, j.float(), mask), 1)[conf.view(-1) > conf_thres]
