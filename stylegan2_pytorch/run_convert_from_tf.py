...
def get_arg_parser():
    parser = argparse.ArgumentParser(
        ...
        epilog='Pretrained gan_models that can be downloaded:\n{}'.format(
    ...
    parser.add_argument(
        '-i',
        '--input',
        help='File path to pickled tensorflow gan_models.',
        type=str,
        default=None,
    )
    ...
    parser.add_argument(
        '-d',
        '--download',
        help='Download the specified pretrained model. Use --help for info on available gan_models.',
        type=str,
        default=None,
    )
    ...
    parser.add_argument(
        ...
        help='One or more output file paths. Alternatively a directory path ' + \
            'where all gan_models will be saved. Default: current directory',
        type=str,
        nargs='*',
        default=['.'],
    )
...
def main():
    ...
    if args.input:
        ...
        assert args.download in pretrained_model_urls.keys(), \
            'Unknown model {}. Use --help for list of gan_models.'.format(args.download)
        unpickled = load_tf_models_url(pretrained_model_urls[args.download])
    if not isinstance(unpickled, (tuple, list)):
        unpickled = [unpickled]
    print('Converting tensorflow gan_models and saving them...')
    ...
    if len(args.output) == 1 and (os.path.isdir(args.output[0]) or not os.path.splitext(args.output[0])[-1]):
        ...
        assert len(args.output) == len(converted), 'Found {} gan_models '.format(len(converted)) + \
