...
class DetectorYolov3():
    def __init__(self, cfgfile="PyTorchYOLOv3/config/yolov3.cfg", weightfile="PyTorchYOLOv3/weights/yolov3.weights", show_detail=False, tiny=False, epoch_save=-1, multi_score=True):
        #
        start_t = time.time()
        ...
        print(f'Loading Tiny-YOLOv3 weights from %s... Done!' % (weightfile)) if tiny else print(f'Loading YOLOv3 weights from %s... Done!' % (weightfile))
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        if self.device == "cuda":
            self.use_cuda = True
            self.model.cuda()
    ...
    def detect(self, input_imgs, cls_id_attacked, epoch, with_bbox=True):
        ...
        if not(detections[0] == None):
            ...
            if with_bbox:
                ...
                bboxes = non_max_suppression(c_detections, 0.4, 0.6) ## <class 'list'>.
        ...
        if with_bbox:
            ...
            return max_prob_obj_cls, output_class_target, bboxes
        else:
