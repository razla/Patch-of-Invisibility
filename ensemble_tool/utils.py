...
def save_samples_simple(index, latent_tensors, generator, sample_dir, show=False):
    ...
    if show:
        ...
        ax.imshow(make_grid(fake_images.cpu().detach(), nrow=8).permute(1, 2, 0))
        plt.show()
def save_samples_GANLatentDiscovery(index, sample_dir, G,
                                    latent_vector,
                                    device):
    '''
    latent_shift: latent_shift_biggan
    param_rawPatch_latent: alpha, ex:0.99
    param_rawPatch_latent: rand
    '''
    z = latent_vector
    interpolation_deformed = interpolate_shift(G, z.unsqueeze(0),
                            latent_vector=torch.zeros_like(z).to(device),
                            device=device)
